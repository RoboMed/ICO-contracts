...
*/
  ...
  */
  ...
  /**
  * Здесь храним начисленные премиальные токены, могут быть выведены на кошелёк начиная с даты startDateOfUseBountyTokens
  */
  ...
  */
  ...
  /**
   * Количество выпущенных премиальных токенов
   */
  uint256 public bountyAndTeam = 0;

  /**
  * Дата, начиная с которой можно получить bounty токены непосредственно на кошелёк
  */
  uint256 public startDateOfUseBountyTokens = 0;
   ...
   * только если ico закончилось или вызов осуществляется владельцем
   */
  modifier afterIco() {
  ...
  */
  ...
  /**
  * Метод зачисляющий предварительно распределённые baunty токены на кошелёк
  */
  function accrueBounty() afterIco {
    //зачисление возможно только после определённой даты
    require(startDateOfUseBountyTokens <= now);

    //зачисляем на кошелёк и обнуляем не начисленные
    balances[msg.sender] = balances[msg.sender].add(bountyBalances[msg.sender]);
  ...
  */
  ...
  function gotoPostIco() private returns (bool) {
    ...
    //определяем количество премиальных токенов
    bountyAndTeam = (totalBought + INITIAL_COINS_FOR_VIPPLACEMENT) / 4;
    ...
    startDateOfUseBountyTokens = now + DURATION_NONUSEBOUNTY;

    //проводим распределение Bounty и TeamTokens
    uint256 localBountyAndTeam = bountyAndTeam;

    //member1
    localBountyAndTeam = reward(0x0, (bountyAndTeam / 10), localBountyAndTeam);

    //member2
    localBountyAndTeam = reward(0x0, (bountyAndTeam / 10), localBountyAndTeam);

    //last member
    localBountyAndTeam = reward(0x0, localBountyAndTeam, localBountyAndTeam);
  ...
  /**
   * Вспомогательный метод выполняющий премирование участника
   */
  function reward(address rewardAddr, uint256 rewardValue, uint256 all) private returns (uint256) {
    bountyBalances[rewardAddr] = rewardValue;
    totalSupply = totalSupply.add(rewardValue);
    return all.sub(rewardValue);
  }
