...
может быть запущено owner после наступления pastico но через определенное минимальное время
...
saleStageLast (3) 550
postIco
postIcoSaleStage1 100 100
postIcoSaleStage2 200 200
postIcoSaleStage3 300 250
postIcoSaleStage4 300
postIcoSaleStage5 400
postIcoSaleStage6 200
postIcoSaleStage7 100
postIcoSaleStageLast -
...
/**
 * Модификатор позволяющий выполнять вызов,
 * только если состояние находиться перед PostIco
 */
modifier beforeIco() {
require(uint(currentState) < uint(IcoStates.PostIco));
_;
...
if (
uint(currentState) >= IcoStates.SaleStage1
&&
uint(currentState) <= IcoStates.SaleStage7) {
...
//если покупка была выполнена на любой стадии PostIcoSale кроме последней
